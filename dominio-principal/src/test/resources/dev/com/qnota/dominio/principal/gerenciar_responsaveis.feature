Feature: Gerenciar responsáveis
	...
	Scenario: Bloquear cadastro com CPF em formato inválido (falha)
		Given um "responsável" com CPF "12345678909" "não está" registrado
	...
	Scenario: Bloquear cadastro de responsável com CPF já existente (falha)
		Given um "responsável" com CPF "987.654.321-00" "já está" registrado
		When um coordenador tenta cadastrar outro "responsável" com o CPF "987.654.321-00"
		Then o sistema rejeita o cadastro
		And o sistema informa que "já existe responsável com esse CPF"

	Scenario: Cadastrar responsável com CPF inédito (sucesso)
		Given não existe "responsável" com CPF "741.852.963-10"
		When um coordenador cadastra o "responsável" com CPF "741.852.963-10"
		Then o sistema confirma o cadastro do "responsável"
	...
	Scenario: Bloquear cadastro sem e-mail (falha - NOT BLANK)
		...
		When um coordenador tenta cadastrar o "responsável" com nome "Carla" e CPF "295.379.650-40"
		Then o sistema rejeita o cadastro
		And o sistema informa que "'email' não pode ser vazio"

	Scenario: Cadastrar responsável com e-mail preenchido (sucesso - NOT BLANK)
		Given um "responsável" "não está" registrado
		When um coordenador cadastra o "responsável" com nome "Carla", CPF "295.379.650-40" e e-mail "carla@exemplo.com"
		Then o sistema confirma o cadastro do "responsável"
	...
	Scenario: Cadastrar responsável com CPF somente dígitos (sucesso - formato aceito)
		Given um "responsável" "não está" registrado
		When um coordenador cadastra o "responsável" com nome "Elaine", CPF "74185296310" e e-mail "elaine@exemplo.com"
		Then o sistema confirma o cadastro do "responsável"
	...
	Scenario: Vincular responsável ainda não associado ao aluno (sucesso - RN-20)
		...
		When um coordenador vincula o "responsável" "R2" ao "aluno" com grau "Mãe" e "principal"
	...
	# --------- Validação básica no VÍNCULO (NOT BLANK em grau de parentesco) ---------
